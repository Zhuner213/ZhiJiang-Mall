          ...
          <div class="order-content">
            ...
            <div class="orders">
              ...
              >
                ...
                <tbody>
                  <tr v-for="(good,index) in order.orderDetailList" :key="good.skuId">
                    <td width="60%">
...
export default {
  ...
  methods: {
    // 获取我的订单列表
    async getData() {
      const response = await this.$api.reqGetOrderList(this.pageNo, this.pageSize);
  ...
  mounted() {
    // 页面挂载完毕就获取一下我的订单列表
    this.getData();
    // 页面挂载完毕注册一下分页器使用的全局事件总线的事件
    // 从分页器子组件收到页码，然后向服务器发请求获取该页码的数据
