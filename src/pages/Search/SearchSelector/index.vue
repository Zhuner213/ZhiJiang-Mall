...
import {mapGetters} from 'vuex'
  export default {
    ...
    methods: {
      // 这里要在父组件发请求，因为父组件中的 searchParams 是带给服务器的参数，而且数据是在父组件中展示的
      trademarkHandler(trademark) {
        this.$bus.$emit('getTrademark', trademark)
      },
      // 点击商品售卖属性的属性值，把该属性的相关值都传给父组件，让父组件利用这些值发请求 
      // 在这里就提前按照父组件需要的格式把值整理好
      attrHandler({attrId, attrName}, attrValue) {
        const props = `${attrId}:${attrValue}:${attrName}`
        this.$bus.$emit('getProps', props)
      }
    },
  }
</script>
